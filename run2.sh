...
function curles()
{
	...
	_VAR=$(grep -oP 'ELASTIC_PASSWORD=\K(.*)' variables)
curl --trace-ascii /dev/stdout -u "elastic:${ELASTIC_PASSWORD}" $* 
...
-u elastic:${ELASTIC_PASSWORD%%[[:cntrl:]]} --data-binary @hsh_policy.json

curl -k https://${_IP}:9200/_component_template/hsh_component -XPUT -H "Content-Type: application/json" \
-u elastic:${ELASTIC_PASSWORD%%[[:cntrl:]]} --data-binary @hsh_component.json

curl -k https://${_IP}:9200/_index_template/power_consume -XPUT -H "Content-Type: application/json" \
-u elastic:${ELASTIC_PASSWORD%%[[:cntrl:]]} --data-binary @hsh_power.json
